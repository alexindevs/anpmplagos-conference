...
function successMessage(kind: string | undefined): string {
  switch (kind) {
    ...
    case "registration":
      return "Your registration is now confirmed. You can access your conference ticket.";
    default:
...
function MemberPaymentCallbackContent() {
  const { data: user } = useAuthSession();
  const queryClient = useQueryClient();
  const searchParams = useSearchParams();
  const reference = searchParams.get("reference") || searchParams.get("trxref");

  const fullName = user?.member?.fullName || "Member";
  const userId = user?.id || "";

  const { data: result, isError, error, isPending, isFetching } = useQuery({
    queryKey: ["payment", "verify", "member-callback", reference],
  ...
  useEffect(() => {
    ...
    if (payment.status === "success") {
      if (payment.kind === "hotel_room") {
        ...
        void queryClient.invalidateQueries({ queryKey: ["member", "event-passes"] });
            ...
            <div className="flex flex-col gap-3">
              {isSuccess && (
                <>
                  ...
                  <Link
                    href={kind === "hotel_room" ? "/hotel-rooms" : "/member/dashboard"}
...
export default function MemberPaymentCallbackPage() {
  return (
    <Suspense fallback={<PaymentCallbackSuspenseFallback />}>
      <MemberPaymentCallbackContent />
    </Suspense>
  );
}
