...
export default function CompanySupportNewPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (res) => {
      router.push(`/company/support/${res.id}`);
    },
    onError: (e: unknown) => {
  ...
  return (
    <>
      <header className="border-b border-secondary/15 bg-background-light/95 px-4 py-5 backdrop-blur sm:px-6 sm:py-6 lg:px-8">
        <div className="border-l-4 border-secondary pl-4">
          <Link
            href="/company/support"
            className="mb-2 inline-flex items-center gap-1 text-sm font-semibold text-secondary hover:underline"
          >
      ...
      <div className="flex-1 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <form
          ...
          className="mx-auto  w-full max-w-[80%] md:max-w-[50%] space-y-6 rounded-xl border border-secondary/20 bg-white p-6 shadow-sm"
        >
          {formError ? (
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-secondary focus:ring-1 focus:ring-secondary"
              maxLength={200}
              required
            />
          </div>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <button
              ...
              className="rounded-lg bg-secondary px-6 py-2.5 text-sm font-bold text-white transition-colors hover:brightness-110 disabled:opacity-50"
            >
