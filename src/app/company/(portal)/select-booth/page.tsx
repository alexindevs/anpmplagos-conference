...
export default function SelectBoothPage() {
  ...
  const payMutation = useMutation({
    mutationFn: (boothId: string) => initializeBoothPayment({ boothId }),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["company", "booths", "available"] });
      queryClient.invalidateQueries({ queryKey: ["company", "dashboard"] });
      queryClient.invalidateQueries({ queryKey: ["company", "me", "booth"] });
  ...
  const { data: publicExhibitors = [], isLoading: publicListLoading } = useQuery({
    queryKey: ["companies", "public", "list", "select-booth"],
    queryFn: getPublicExhibitors,
  });

  const { data: exhibitorProfile } = useQuery({
    queryKey: ["company", "me", "profile", "select-booth"],
    ...
    <main className="flex-1 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
          ...
          <a
            ...
            className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-secondary/30 bg-white px-4 py-2.5 text-sm font-bold text-secondary shadow-sm transition-colors hover:bg-secondary/10 sm:w-auto"
          >
              ...
              {myBooth ? (
                ...
                <>
                  <button
                    ...
                    disabled={!selectedBoothId || !companyId || payMutation.isPending}
                  ...
                  <button
                    type="button"
                    onClick={goToDashboard}
                    className="mt-3 w-full text-center text-sm font-bold text-secondary hover:underline"
                  >
                    Go to dashboard
                  </button>
                  <p className="text-xs text-slate-500 text-center mt-3">
