...
export default function ExhibitorDashboardPage() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        return await getExhibitorReviews();
      } catch (e) {
        if (e instanceof ApiError && e.status === 404) return [];
        throw e;
      }
  ...
  return (
    <>
      <main className="flex-1 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
          ...
          <aside className="xl:col-span-1">
            <section className="rounded-xl border border-secondary/20 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <button
                  type="button"
                  onClick={() => dispatchModal({ type: "OPEN_ADD_REVIEW" })}
                  ...
                  title="Leave a review of the conference"
                >
        ...
        <section className="mt-6 rounded-xl border border-secondary/20 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <h2 className="text-lg font-black text-charcoal">Your Reviews</h2>
            <p className="text-xs text-slate-500">
              Approved reviews appear publicly on our sponsors page
            </p>
          </div>

          {!companyId ? (
            <p className="text-sm text-slate-600 py-6 text-center">
              Your reviews will appear here once your account is linked.
            </p>
          ) : reviewsLoading ? (
            <div className="space-y-3 py-2">
              {[1, 2].map((i) => (
            ...
            <div className="rounded-xl border border-dashed border-secondary/25 bg-slate-50/80 py-10 text-center">
              ...
              <p className="text-sm font-bold text-charcoal mt-2">No reviews yet</p>
              <p className="text-xs text-slate-500 mt-1 mx-auto">
                Share your experience sponsoring this conference — it may be featured publicly.
              </p>
            </div>
          ) : (
