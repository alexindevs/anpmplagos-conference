...
import { verifyPayment, formatKoboToNaira } from "@/lib/api";
import { useAuthSession } from "@/hooks/use-auth-session";
import { exhibitorBoothPaymentResultKey } from "@/lib/company-local-storage";

function BoothPaymentCallbackContent() {
  ...
  useEffect(() => {
    ...
    const exhibitorId =
      ...
      (me?.exhibitor?.id && me.exhibitor.id.trim()) ||
      "";
    try {
      ...
      if (exhibitorId) {
        localStorage.setItem(exhibitorBoothPaymentResultKey(exhibitorId), payload);
        localStorage.removeItem("exhibitorBoothPaymentResult");
      }
    } catch {
      /* ignore */
    }
    if (payment.status === "success" && payment.kind === "booth") {
      void queryClient.invalidateQueries({ queryKey: ["exhibitor", "dashboard"] });
      void queryClient.invalidateQueries({ queryKey: ["exhibitor", "me", "booth"] });
      void queryClient.invalidateQueries({ queryKey: ["exhibitor", "booths", "available"] });
    }
      ...
      <div className="max-w-2xl w-full">
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          ...
          <p className="text-center text-gray-600 mb-8">
            {isSuccess && payment?.kind === "booth" && "Your booth has been successfully reserved. You can now manage your exhibition space."}
            {isSuccess && payment?.kind !== "booth" && "Your payment has been processed successfully."}
            {isFailed && "Your payment could not be completed. Please try again or contact support."}
            {isRefunded && "Your payment has been refunded. The booth may no longer be available."}
              ...
              {payment.baseAmount && (
                <div>
                  ...
                  <p className="text-2xl font-black text-primary">
                    {formatKoboToNaira(payment.baseAmount)}
                  </p>
                </div>
              )}
...
export default function BoothPaymentCallbackPage() {
  return (
    <Suspense fallback={<BoothPaymentCallbackFallback />}>
      <BoothPaymentCallbackContent />
    </Suspense>
  );
}
